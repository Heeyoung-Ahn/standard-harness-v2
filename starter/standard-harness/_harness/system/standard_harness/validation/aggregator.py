    ...
    def validate_starter(self) -> list[dict[str, Any]]:
        return [
            _diagnostic(
                ...
                repair_hint=str(diagnostic["repair_hint"]),
                affected_entity_type=str(diagnostic["affected_entity_type"]),
                affected_entity_id=str(diagnostic["affected_entity_id"]),
            )
...
def _default_starter_root(cwd: Path) -> dict[str, object]:
    repository_starter = cwd / "starter" / "standard-harness"
