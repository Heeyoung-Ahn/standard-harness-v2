...
| User requested project-start Conductor and per-packet worker1/worker2 | provider topology packet | covered by PKT-25 | Planner, then Orchestrator after RFC |
...
1. Provider topology is partially represented today through local/ad hoc command fields,
   not a first-class starter contract. PKT-25 owns the schema, persistence, CLI, and
   real-smoke validation correction.
...
- `.agents/runtime/ACTIVE_CONTEXT.json` currently routes to Planner with no active packet.
- Generated state is a read model only and was not manually edited.
- `reference/packets/PKT-24_SURVEY_APP_WEB_REVIEW.md` exists as an untracked product
  survey-app packet and is intentionally not reused for these harness improvements.
