...
## Repository Mission And Entry Scope

This file is a Codex-only development entry contract for this repository. It is not
part of the clean starter payload, and it must not be copied into
`starter/standard-harness/`.
This file is not part of the clean starter payload.

The product being built in this repository is the clean Standard Harness payload under
`starter/standard-harness/`. All product-facing harness work should improve that
payload unless the Human Owner explicitly asks for development-repository tooling,
tests, or release infrastructure.

The repository root contains development scaffolding used to build, test, and release
the starter payload. Root-level `src/`, `tests/`, `tools/`, `docs/`, `_ops/`, and this
`AGENTS.md` are not automatically product payload. They may inform or verify payload
changes, but starter-safe behavior must be promoted into `starter/standard-harness/`
through explicit, validated changes.

Do not add `starter/standard-harness/AGENTS.md`, `AGENT.md`, `CLAUDE.md`, `GEMINI.md`,
or another provider-specific entry contract to the starter payload. Standard Harness v2
must remain provider-neutral: Codex, Claude Code, Antigravity, IDE tools, browser tools,
device tools, and cloud execution systems should connect through harness policies,
contracts, adapters, routing, and evidence protocols rather than defining the harness
as a child of any one LLM runtime.

Starter-facing onboarding belongs in `starter/standard-harness/START_HERE.md` only when
it explains how a new project uses a copied clean payload. Harness contracts belong in
`starter/standard-harness/_harness/contracts/`, policies in
`starter/standard-harness/_harness/policies/`, and executable validation in
...
8. Update `_harness/contracts/hr-coverage-matrix.yaml`.
