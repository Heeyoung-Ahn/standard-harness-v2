...
class RequirementsMetadataRepository:
    ...
    def requirements_index(self) -> dict[str, Any]:
        return self._read_jsonish(self.path("requirements", "requirements-index.yaml"))

    def traceability_matrix(self) -> dict[str, Any]:
        return self._read_jsonish(self.path("requirements", "traceability-matrix.yaml"))

    def hr_coverage_matrix(self) -> dict[str, Any]:
        return self._read_jsonish(self.path("requirements", "hr-coverage-matrix.yaml"))
    ...
    def required_artifact_paths(self) -> dict[str, Path]:
        return {
            "requirements-index": self.path("requirements", "requirements-index.yaml"),
            "traceability-matrix": self.path("requirements", "traceability-matrix.yaml"),
            "hr-coverage-matrix": self.path("requirements", "hr-coverage-matrix.yaml"),
